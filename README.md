...
- **Paredes** (madeira/pedra/ferro) e **Porteira** ($100, 100 de vida: jogadores atravessam, zumbis não). Derrubar: segure **E** com a ferramenta certa — **machado** na madeira (3 golpes) e na porteira (2), **picareta** na pedra (5) e no ferro (8). Regras em `shared/items.ts` (`WALL_TOOL`, `WALL_HITS`, `WALL_PASSABLE`) e `Match.hitWall`.
- Cada chefão morto deixa o **Coração do Chefão** no chão (vende por $300 no vendedor).
- **5º chefão morto**: todos na partida ganham um **🏆 troféu** (persistido; aparece ao lado do nome no mundo, no lobby e no menu) e um **helicóptero de resgate** pousa ao lado da antena em 6 s. Chegue a 2,2 m dele para embarcar (some do mundo, invulnerável). Com todos a bordo — ou 90 s depois de pousar — ele decola: cutscene com a câmera no helicóptero e os **créditos** sobem (`evac_complete` lista resgatados e quem ficou). Regras em `GAME.evac`, `Match.callHelicopter/tickEvac`; visual em `client/src/World/Helicopter.ts` e `UI/CreditsUI.ts`.
